test_that("prep_param_files()", {
  ...
  run_engine_contin(params, n_sims = 2, verbose = FALSE)
  out2 <- run_flfll(facts_file, verbose = FALSE)
  run_engine_contin(out2, n_sims = 2, verbose = FALSE)
  out3 <- run_flfll(facts_file, verbose = FALSE)
  params3 <- prep_param_files(out3)
  lapply(params3, run_engine_contin, n_sims = 2, verbose = FALSE)
