...
To install the latest GitHub version of `rfacts`, open R and run the following.
...
First, create a `*.facts` XML file using the [FACTS](https://www.berryconsultants.com/software/) GUI. The `rfacts` package has several built-in examples.
...
rfacts makes it straightforward to parallelize across simulations. First, use `run_flfll()` to create a directory of param files. Be sure to supply an `output_path` that all the parallel workers can access (e.g. no `tempfile()`s).
